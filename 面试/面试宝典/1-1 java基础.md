...
为什么那么int类型的数据范围负数比正数多1？

int类型的数据用补码表示，对于正数来说表示方式同原码相同，最大数为01111111111111111111111111111111 = 2^31 - 1。

负数是对应大小的原码表示除了符号位之外，取反得到反码再加1，原码最小数为 原码 11111111111111111111111111111111 = 补码 10000000000000000000000000000001 = - (2^31 - 1) 。

在此基础上，没有一个原码转补码后为10000000000000000000000000000000，便约定这个数为最小的补码负数 -2^31。于是int类型的数据范围是 -2^31 ~ 2^31-1。

也可以理解为补码中 0 的表示唯一，不区分正负，均为 00000000000000000000000000000000。而原码0有两个，+0和-0。

byte,short,long整数类型同理。
...
[复制代码](https://www.nowcoder.com/study/live/537/2/2#)
...
补充解释 1.18 处重写（override）的 “返回值要小于等于父类方法” 的意思，下面举两个例子

```java
class Test extends Test2_3 {
    @Override
    Double test() {
...
Double类是Object类的子类，而Test类是Test2_3的子类，所以此处“返回值要小于等于父类方法”的意思是指返回值之间的父子关系。而不是 float 与 double 这些的关系（一开始理解错了）。
